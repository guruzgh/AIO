#!/usr/bin/env bash
#
# Try `install-hysteria.sh --help` for usage.
#
# (c) 2025 Eddyme23
#
...
setup_ssl() {
        ...
        openssl x509 -req -extfile <(printf "subjectAltName=DNS:%s,DNS:%s" "$DOMAIN" "$DOMAIN") -days 3650 -in /etc/hysteria/hysteria.server.csr -CA /etc/hysteria/hysteria.ca.crt -CAkey /etc/hysteria/hysteria.ca.key -CAcreateserial -out /etc/hysteria/hysteria.server.crt
 }
start_services() {
