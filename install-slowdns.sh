...
install_slowdns() {
  ...
  suggested_ns="ns.${domain}"
  read -p "Enter your Nameserver (or press enter to use default: ${suggested_ns}): " Nameserver
  if [ -z "$Nameserver" ]; then
    Nameserver="$suggested_ns"
  fi
...
ExecStart=/etc/slowdns/dns-server -udp :5300 -privkey-file /etc/slowdns/server.key $Nameserver 127.0.0.1:109
Restart=on-failure
